...
class listener implements EventSubscriberInterface
{
	 ...
	 */
	public function __construct(\phpbb\template\template $template, \phpbb\user $user, \phpbb\config\config $config, \phpbb\db\driver\driver_interface $db, \phpbb\auth\auth $auth, \phpbb\event\dispatcher_interface $dispatcher, $table_prefix, $phpbb_root_path, \phpbb\cache\driver\driver_interface $cache = null)
	{
	...
	public function on_page_header($event)
	{
		...
		// Render blocks
		$cache_name = '_vinny_sidebar_blocks';
		$blocks = ($this->cache !== null) ? $this->cache->get($cache_name) : false;

		if ($blocks === false)
		{
			$sql = 'SELECT * FROM ' . $this->table_prefix . 'vinny_sidebar_blocks WHERE block_enabled = 1 ORDER BY block_order ASC';
			$result = $this->db->sql_query($sql);
			$blocks = $this->db->sql_fetchrowset($result);
			$this->db->sql_freeresult($result);

			if ($this->cache !== null)
			{
				$this->cache->put($cache_name, $blocks);
			}
		}
		...
		foreach ($blocks as $row)
		{
			...
			if ($row['block_is_system'])
			{
				switch ($row['block_name'])
				{
					...
					case 'SIDEBAR_CLOCK':
						...
						$this->template->assign_vars([
							'SIDEBAR_CLOCK_FORMAT' => $clock_format,
						]);
						break;
			...
			{
				$parse_bbcode = isset($row['block_parse_bbcode']) ? (bool) $row['block_parse_bbcode'] : true;
				if ($parse_bbcode && !empty($row['block_content']))
				{
					$bbcode_uid = $row['bbcode_uid'] ?? '';
					$bbcode_bitfield = $row['bbcode_bitfield'] ?? '';
					$bbcode_options = isset($row['bbcode_options']) ? (int) $row['bbcode_options'] : 7;
			...
			/**
			 * Event to modify a sidebar block before display
			 *
			 * @event vinny.sidebar.render_block
			 * @var array  block_data  Template data assigned to the sidebar block (TITLE, CONTENT, TEMPLATE_FILE, S_DISPLAY)
			 * @var array  row         Raw database row for the current sidebar block
			 * @since 1.1.0
			 */
			$vars = ['block_data', 'row'];
			extract($this->dispatcher->trigger_event('vinny.sidebar.render_block', compact($vars)));

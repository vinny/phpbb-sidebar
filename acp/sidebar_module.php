			...
			if ($action == 'add' || $action == 'edit')
			{
				...
				if ($submit || $preview || $analyse)
				{
					...
					if ($parse_bbcode)
					{
						...
						$uid = $bitfield = '';
						// Build options bitmask and parse text for storage (based on core acp_contact.php pattern)
						...
						generate_text_for_storage($storage_content, $uid, $bitfield, $flags, $allow_bbcode, $allow_urls, $allow_smilies);
				...
				$template->assign_vars([
					...
					// BBCode status labels and permissions (based on core acp_contact.php pattern)
